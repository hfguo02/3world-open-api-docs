...
- 默认文档是 V2，内容位于 `docs/`。V2 包含 V2 替代接口，以及仍然有效、尚未被替代的 V1 路径；已被替代的 V1 接口不得出现。
- V1 是完整历史快照，位于 `versioned_docs/version-1.0.0/`，路由前缀为 `/v1`。从 V2 打开 V1 时必须使用新窗口，并显示明确的 Legacy 标识。
- 接入指南、API 文档、Webhook 事件、参考资料必须属于同一个 Docusaurus 文档实例和同一套侧栏壳层；禁止重新拆成多个内容插件或用下拉菜单拼接路由。
- 一次对接不得混用两个文档版本。搜索必须沿用 Docusaurus 版本上下文，V1 与 V2 不共享结果集。

## 目录与生成边界

- 人工内容只维护在 `docs/introduction/`、`docs/webhooks/`、`docs/reference/` 和对应的历史版本快照中。
- `docs/api/`、`versioned_docs/version-1.0.0/api/`、`openapi/whitelabel/releases/`、`sidebars.ts`、`versioned_sidebars/` 都是生成产物，不手工修改。
...
- Copy Page 使用 `docusaurus-plugin-copy-page-button`；LLM 索引使用 `docusaurus-plugin-llms`。`LLMs.txt` 不出现在页面 UI，但 `/llms.txt` 与 `/v1/llms.txt` 必须随两个版本持续生成和验证，不能因入口不可见而停止维护。
...
2. `npm run typecheck`
3. `npm run build`
4. 用真实浏览器检查 `/introduction`、`/api`、一个 API 详情页、`/v1/introduction`、V1 新窗口、Copy Page、搜索和明暗主题。
